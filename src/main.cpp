...
void validateAndHandleInput(mState state, const std::vector<std::string>& cmd_tokens) {
    try {
        ...
        // Modify the state or perform actions accordingly

        switch (state) {
            case IDLE:
                if (cmd_tokens.size() > 1) {
                    std::cout << "IDLE has no arguments." << std::endl;
                    INFO_REL("Arguments count" << cmd_tokens.size());
                    return;
                }
                break;
            default:
        ...
        std::cout << "Invalid argument. Please enter a valid value." << std::endl;
    } catch (const std::out_of_range& e) {
        std::cout << "Argument out of range. Please enter a valid value." << std::endl;
    }
}
void reset_terminal() {
...
std::string get_available_commands(const std::unordered_map<std::string, std::pair<mState, std::string>>& command_map) {
    ...
    for (const auto& entry : command_map) {
        maxCommandLength = std::max(maxCommandLength, entry.first.length());
    }

    oss << "\r";
    for (const auto& entry : command_map) {
        oss << std::left << std::setw(maxCommandLength + 2) << entry.first + ": " << entry.second.second << "\n";
    }
    oss << "\r";
...
void decode_cmd(std::string cmd, mState& state, mState& prev_state, std::condition_variable& state_cv){
    ...
    if (iter != command_map.end()) {
        ...
        validateAndHandleInput(state, cmd_tokens);
    }   
    else {
        prev_state = state;
        state = HELP;
